...
MAX_DSTEER: float = float(np.deg2rad(45.0)) # 180  # maximum steering speed [rad/s] (i.e. AckermannDrive.steering_angle_velocity)
MAX_SPEED: float = 15.0  # maximum speed [m/s]
MAX_ACCEL: float = 3.0 #10.0   # maximum acceleration [m/ss]

# Weights for tuning

Rk = [0.005, 80.0] # 0.001 #([0.001, 90.0]) without first wp's speed
#np.diag([0.001, 90.0])
Rdk = [0.1, 750.0] # without first wp's speed: ([0.1, 500.0])
#default_factory=lambda: np.diag([0.1, 750.0])
#np.diag([0.01, 100.0])
#np.diag([0.1, 300.0])
#np.diag([0.1, 400.0])
#np.diag([0.1, 900.0])
